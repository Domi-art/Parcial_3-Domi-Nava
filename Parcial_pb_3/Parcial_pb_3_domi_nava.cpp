...
void mostrarCarritoArchivo()
{
    ...
    {
        stringstream ss(linea);
        string idCarrito, idUsuario, nombreUsuario, idProd, nombreProd, cantidad, precio, subtotal;
...
void mostrarComentariosDesdeFecha(const vector<Comentario> &comentarios, const string &fechaLimite)
{
    ...
    if (!encontrado)
    {
        cout << "No se encontraron comentarios despues de esa fecha.\n";
    }
}
...
void listarProductosCarritoPorUsuario()
{
    ...
    {
        stringstream ss(linea);
        string idCarrito, idUsuario, nombreUsuario, idProducto, nombreProducto, cantidad, precio, subtotal;
...
void generarOrdenDeCompra(const Usuario &usuario, vector<ItemCarrito> &carrito, int &contadorOrdenes, vector<BonoUsuario> &bonos)
{
    if (carrito.empty())
    {
...
void reporteProductosMenorStock(vector<Producto> productos) {
    if (productos.empty()) {
    ...
    for (size_t i = 0; i < productos.size(); ++i) {
        for (size_t j = i + 1; j < productos.size(); ++j) {
            if (productos[j].stock < productos[i].stock) {
    ...
    size_t limite = productos.size() < 5 ? productos.size() : 5; 
    for (size_t i = 0; i < limite; ++i) {
        cout << productos[i].id << " | " << productos[i].nombre << " | " << productos[i].stock << "\n"; 
    }
}
...
void reporteComentariosPorFecha(const vector<Comentario>& comentarios) {
    ...
    if (!fechaBusqueda.empty() && fechaBusqueda[fechaBusqueda.size() - 1] == '\r') {
        fechaBusqueda.erase(fechaBusqueda.size() - 1);
    }
    ...
    cout << "\n=== INFORME DE COMENTARIOS POR FECHA ===\n";
    cout << "fecha | cantidad/numero de comentarios\n"; 
    cout << fechaBusqueda << " | " << contador << "\n"; 
}
...
void reportePrecioMaxMin(const vector<Producto>& productos) {
    ...
    for (size_t i = 1; i < productos.size(); ++i) {
        if (productos[i].precio < minPrecio) minPrecio = productos[i].precio;
        if (productos[i].precio > maxPrecio) maxPrecio = productos[i].precio;
    }

    cout << "\n=== INFORME DE PRECIOS MAXIMO Y MINIMO ===\n"; 
    cout << "Precio Minimo: $" << minPrecio << "\n";
    cout << "Precio Maximo: $" << maxPrecio << "\n";
}
...
int main()
{
    ...
    do
    {
        ...
        cout << "10. Listar productos mas vendidos\n";
        cout << "11. Listar ganancias de la plataforma\n";
        cout << "12. Reportes Adicionales (Parcial Final)\n"; 
        cout << "0. Salir\n";
        ...
        {
            string fechaBusqueda;
            cout << "Ingrese la fecha limite (Formato AAAA-MM-DD, ej: 2025-06-01): ";
        ...
        {
            int opcionReporte;
            cout << "\n--- SUBMENU DE REPORTES ADICIONALES ---\n"; 
            cout << "1. Listado de 5 productos con menor stock (Ascendente)\n"; 
            cout << "2. Cantidad de comentarios por fecha especifica\n"; 
            cout << "3. Precio maximo y minimo de productos\n"; 
            cout << "Seleccione un informe: ";
            cin >> opcionReporte;
            cin.ignore();
            ...
            switch (opcionReporte)
            {
                case 1:
                    ...
                    break;
                case 2:
                    reporteComentariosPorFecha(listaComentarios); 
                    break;
                case 3:
